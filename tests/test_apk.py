...
def test_bundled_quickbar_apk() -> None:
    ...
    with ZipFile(apk) as zf:
        ...
        assert "com.changanhub.qb1_3_20".encode("utf-16-le") in mf
        assert "1.3.20".encode("utf-16-le") in mf
        assert "res/drawable/ic_sign.xml" in names
        dex = zf.read("classes.dex")
        assert b"HuSigner" in dex
        assert b"KeepAliveAccessibility" in dex
        assert b"coverForSystemUi" in dex
        assert b"requestKillPermission" in dex
