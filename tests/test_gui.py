...
def test_gui_pages_render() -> None:
    ...
    try:
        app.root.update()
        for name in ("connect", "install", "ours", "overlay", "player", "aichat", "wifi", "news", "demo", "apps", "catalog", "tools"):
            app.show(name)
...
def test_install_button_visible_at_laptop_size() -> None:
    ...
    try:
        ...
        app.root.update_idletasks()
        app.root.update()
        app.show("connect", "Подключение")
        app.root.update_idletasks()
        app.root.update()
        assert app.connect_btn.winfo_ismapped()
        assert _inside_window(app.connect_btn, app.root)
        ...
        app.root.update_idletasks()
        app.root.update()
        assert app.news_install_btn.winfo_ismapped()
        assert _inside_window(app.news_install_btn, app.root)
        app.show("demo", "Демо")
        app.root.update()
