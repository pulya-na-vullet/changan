...
def test_apk_package_name_quickbar_is_new_id(tmp_path: Path) -> None:
    ...
    assert apk_package_name(tmp_path / "QuickBar.apk") == "com.changanhub.qb1_3_17"
    assert apk_package_name(tmp_path / "QuickBar-changan.apk") == "com.changanhub.qb1_3_17"
    assert apk_package_name(tmp_path / "quicklane.apk") == "com.changanhub.qb1_3_17"
    assert apk_package_name(tmp_path / "quickkeep.apk") == "com.changanhub.qb1_3_17"
    assert apk_package_name(tmp_path / "quickrise.apk") == "com.changanhub.qb1_3_17"
    assert apk_package_name(tmp_path / "quickload.apk") == "com.changanhub.qb1_3_17"
...
def test_install_qb1_3_16_is_first_install_while_qb1_3_15_present(tmp_path: Path) -> None:
    """New overlay id must not hit keep-existing on the leftover versioned panel."""
    apk = tmp_path / "QuickBar.apk"
    with zipfile.ZipFile(apk, "w") as zf:
        zf.writestr("AndroidManifest.xml", b"mf")
        zf.writestr("classes.dex", b"dex")
    ...
    def shell(command: str, timeout: int = 60) -> CommandResult:
        ...
        if command.startswith("pm path com.changanhub.qb1_3_15"):
            return CommandResult(
                True,
                "package:/data/app/com.changanhub.qb1_3_15-T8rRUL7m/base.apk",
                "",
                0,
                [],
            )
        if command.startswith("pm path com.changanhub.qb1_3_16"):
            if any(item.startswith("pm install") for item in fake.shells):
                return CommandResult(
                    True,
                    "package:/data/app/com.changanhub.qb1_3_16-new/base.apk",
                    "",
                    0,
                    [],
                )
    ...
    fake.shell = shell  # type: ignore[method-assign]
    with patch("hub.installer.sign_apk_with_method", return_value=(apk, "python-v1v2")):
        report = install_apk(fake, apk, already_signed=True, package="com.changanhub.qb1_3_16")
    assert report.ok
    assert report.method != "keep-existing"
    assert not any(cmd.startswith("pm uninstall") for cmd in fake.shells)
    assert sum(1 for cmd in fake.shells if cmd.startswith("pm install")) == 1
