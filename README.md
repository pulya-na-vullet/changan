...
Приложение `com.changanhub.qb1_3_20` держит поверх всех Activity узкую колонку
справа:
...
- **Wi-Fi** сразу под названием и версией панели: тап — вкл/выкл (WifiManager, иначе `su -c svc wifi`), шестерёнка / долгое нажатие — системные настройки в окне 10%. Зелёный = включён, серый = выключен;
- **закрыть приложения** (как KillAPK) — красный кружок с крестиком в шапке. В первый раз панель просит **спец. возможности** (колонка прячется, открываются системные настройки). Потом крестик открывает карточку приложения и сам нажимает «Остановить» / Force Stop / 强行停止. `su` не вызывается — на Feiyu это давало тост «ошибка» и ничего не закрывало. Лаунчер, System UI и саму панель не трогает;
...
Приложение `com.changanhub.pl1_1_9` ставится из Hub, раздел **Наши приложения**.
Старые `pl1_1_8` / `pl1_1_7` / `playload` / `playrise` / `lamoreplayer` Feiyu не удаляет: Hub ставит новый id и отключает leftover.
...
- музыка: очередь, шаффл/повтор, теги, EQ (пресеты + BassBoost/Virtualizer/Loudness);
- видео: на весь экран, SRT/ASS рядом с файлом, выбор аудиодорожки;
- заставки: спектр, волна, частицы, круг, OpenGL-туман; реакция на audioSessionId;
- кнопка «Обновить» сканирует USB в ГУ;
- форматы, которые реально играет **декодер Feiyu/MediaTek**: MP3, AAC, M4A,
  FLAC, WAV, OGG, OPUS, MP4 (H.264), MKV/WebM/MOV/TS если внутри тот же кодек.
  WMA/AVI/HEVC/DTS часто не открываются — это ограничение ГУ, не плеера.
...
- автоозвучка ответов встроенным RHVoice (голос Елена внутри APK), без системного TTS Android;
...
ставится новым id `com.changanhub.qb1_3_20`; старые `quickbar` / `quickdock` /
`quicklane` / `quickkeep` / `quickrise` / `quickstash` / `quickload` / `qb1_3_15` / `qb1_3_16` / `qb1_3_17` / `qb1_3_18` / `qb1_3_19` остаются на ГУ, Hub их отключает (`pm disable-user` +
...
панель — после этого «Установить и запустить» ставит `qb1_3_20`. Сброс ГУ до
...
## Wi-Fi в панели и Новости

В развёрнутом QuickBar **под названием и версией** — тот же функционал, что в
[changan_wifi](https://github.com/pulya-na-vullet/changan_wifi): тап включает и
выключает Wi-Fi (`WifiManager.setWifiEnabled`, запасной путь `su -c svc wifi`),
шестерёнка открывает системные настройки. Отдельная плавающая кнопка не нужна —
панель уже поверх всех окон. Готовый APK кнопки по-прежнему лежит в
`apps/WifiButton.apk`, если хотите старый отдельный виджет.

Новости (`ru.changan.news`) сами рисуют поля 10% внутри Activity. Если
`apps/ChanganNews.apk` нет в ZIP, положите `ChanganNews-1.5-release.apk` из
https://github.com/pulya-na-vullet/changan_news
