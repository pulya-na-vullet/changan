...
Приложение `com.changanhub.quickload` держит поверх всех Activity узкую колонку
справа:
...
Приложение `com.changanhub.playload` ставится из Hub, раздел **Наши приложения**.
Старые `playrise` / `lamoreplayer` Feiyu не удаляет: Hub ставит новый id и отключает leftover.
...
Приложение `com.changanhub.chatload` ставится из Hub, раздел **Наши приложения**.
Старые `chatrise` / `aichat` Feiyu не удаляет — Hub ставит новый id.
...
ставится новым id `com.changanhub.quickload`; старые `quickbar` / `quickdock` /
`quicklane` / `quickkeep` / `quickrise` / `quickstash` остаются на ГУ, Hub их отключает (`pm disable-user` +
...
панель — после этого «Установить и запустить» ставит `quickload`. Сброс ГУ до
