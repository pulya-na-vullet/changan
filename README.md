...
Приложение `com.changanhub.qb1_3_19` держит поверх всех Activity узкую колонку
справа:
...
- **закрыть приложения** (как KillAPK) — красный кружок с крестиком в шапке. В развёрнутой панели список запущенных: крестик закрывает одно, «Закрыть сторонние» — все пользовательские. `killBackgroundProcesses`, иначе `su -c am force-stop`. Лаунчер, System UI и саму панель не трогает;
...
ставится новым id `com.changanhub.qb1_3_19`; старые `quickbar` / `quickdock` /
`quicklane` / `quickkeep` / `quickrise` / `quickstash` / `quickload` / `qb1_3_15` / `qb1_3_16` / `qb1_3_17` / `qb1_3_18` остаются на ГУ, Hub их отключает (`pm disable-user` +
...
панель — после этого «Установить и запустить» ставит `qb1_3_19`. Сброс ГУ до
