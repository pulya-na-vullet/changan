...
CATALOG: list[CatalogApp] = [
    CatalogApp(
        id="quickbar",
        name="QuickBar 1.3.20 (правая панель)",
        summary="Всегда поверх всех окон, справа, быстрый запуск установленных приложений.",
        package="com.changanhub.qb1_3_20",
        kind="dock",
        source="bundled",
        notes="Ставится из «Наши приложения». В списке ГУ: QuickBar 1.3.20 · com.changanhub.qb1_3_20. Под названием — Wi-Fi. Красный крестик в шапке — режим KillAPK: в первый раз просит спец. возможности, потом крестик сам нажимает «Остановить». Скрытие, сортировка и свайп «спрятать от дилера» — в зелёной колонке справа. USB или память ГУ: ключ подписывает APK v1+v2 серийником Feiyu (как Hub), зелёная кнопка ставит. Сторонние приложения — кнопка «окно» (freeform 10% от краёв, если ГУ умеет). 2ГИС всегда в этом окне. Старые quickbar / … / qb1_3_19 Feiyu не удаляет (auth) — это не новая панель.",
    ),
    CatalogApp(
        id="player",
    ...
    CatalogApp(
        id="wifi",
        name="Wi-Fi кнопка 1.4",
        summary="Плавающая кнопка вкл/выкл Wi-Fi поверх карты. Штатную кнопку с ГУ убрали.",
        package="com.lamore.wifibutton",
        kind="tools",
        source="bundled",
        url="https://github.com/pulya-na-vullet/changan_wifi",
        notes="APK из changan_wifi, исходники не копируются. Hub выдаёт «поверх окон» и поднимает кнопку. Второй экран: системные настройки Wi-Fi в окне с полями 10%.",
    ),
    CatalogApp(
        id="news",
        name="Новости 1.5",
        summary="RSS-читалка в окне с полями 10% от краёв экрана.",
        package="ru.changan.news",
        kind="web",
        source="bundled",
        url="https://github.com/pulya-na-vullet/changan_news",
        notes="APK из changan_news (репозиторий changan_new). Исходники не копируются. Положите ChanganNews-1.5-release.apk в apps/, если файла ещё нет в ZIP.",
    ),
    CatalogApp(
        id="2gis",
        name="2ГИС",
        summary="Карты. С QuickBar открывается в окне с полями 10%, не на весь экран.",
        package="ru.dublgis.dgismobile",
        kind="nav",
        source="local",
        notes="Свой APK в apps/. Кнопка «окно» в QuickBar и список 2ГИС всегда запускают freeform с отступом 10%. Если Feiyu игнорирует bounds — останется полный экран.",
    ),
    CatalogApp(
        id="files",
        name="Material Files",
...
_EXTRA_LABELS = {
    ...
    "com.changanhub.qb1_3_19": "QuickBar (старая, закрытие через su без разрешения)",
