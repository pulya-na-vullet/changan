...
CATALOG: list[CatalogApp] = [
    CatalogApp(
        id="quickbar",
        name="QuickBar 1.3.17 (правая панель)",
        summary="Всегда поверх всех окон, справа, быстрый запуск установленных приложений.",
        package="com.changanhub.qb1_3_17",
        kind="dock",
        source="bundled",
        notes="Ставится из «Наши приложения». В списке ГУ: QuickBar 1.3.17 · com.changanhub.qb1_3_17. Скрытие, сортировка и свайп «спрятать от дилера» — в зелёной колонке справа. USB или память ГУ: ключ подписывает APK v1+v2 серийником Feiyu (как Hub), зелёная кнопка ставит. Корень флешки тоже читается, в том числе через /mnt/media_rw если /storage/UUID пустой. Свёрнутые кнопки ×2.5. Сторонние приложения — кнопка «окно» (freeform 10% от краёв, если ГУ умеет). 2ГИС и Wi-Fi кнопка всегда в этом окне. Старые quickbar / … / qb1_3_16 Feiyu не удаляет (auth) — это не новая панель.",
    ),
    CatalogApp(
        id="player",
