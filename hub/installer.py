    ...
    if not _ok_install(result) and "update_incompatible" in blob.lower():
        ...
        if keep_conflict and conflict and package and conflict == package:
            ...
            step(
                ...
                "Новые пакеты: com.changanhub.pl1_1_6 и com.changanhub.ch1_0_5.",
                72,
            )
        else:
            step(
...
def _finish_keep_overlay(
    ...
    if kind == "player":
        msg = (
            ...
            "Lamore Player 1.1.6 — пакет com.changanhub.pl1_1_6, раздел «Наши приложения»."
        )
    elif kind == "chat":
        msg = (
...
def _keep_working_overlay(
    ...
    if kind == "overlay":
        ...
        step(
            f"Не обновляю {package}: подпись этого Hub не совпадает с уже стоящей. "
            "Рабочий плеер не отключаю. Новый пакет — com.changanhub.pl1_1_6.",
            72,
        )
    else:
        step(
...
def apk_package_name(apk: Path) -> str | None:
    ...
    if any(token in stem for token in ("player", "lamoreplayer", "playrise", "playload", "pl1_1_3", "pl1_1_4", "pl1_1_5", "pl1_1_6")):
        return "com.changanhub.pl1_1_6"
